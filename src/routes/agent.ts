...
import type { StorageService, IdentityService, AuditService } from '../services/index.js';
...
export function createAgentRoutes(
  ...
  router.get('/retrieve/:id', async (c) => {
    ...
    if (!rl.allowed) {
      auditService.log({
        ...
        action: 'retrieve',
        details: { vaultId: vault.vaultId, success: false, error: 'rate_limit_exceeded' },
      });
      return c.json(
        { error: 'rate_limit_exceeded', resetAt: rl.resetAt, remaining: 0 },
        429,
    ...
    if (!result.success) {
      auditService.log({
        agentId: vault.agentId,
        action: 'retrieve',
        details: { vaultId: vault.vaultId, pieceCid: vault.pieceCid, success: false, error: result.error },
      });
      return c.json(
    ...
    if (!config.x402.mock) {
      const paymentId = payment.nonce;
      settlementTracker.track(paymentId, vault.agentId, `/agent/retrieve/${id}`);
      withRetry(() => x402Client.settlePayment(payment, requirements))
        .then(() => {
          settlementTracker.updateStatus(paymentId, 'settled');
          auditService.log({
  ...
  router.post('/export-registry', async (c) => {
    if (!config.identity.enabled) {
      return c.json(
        { error: 'identity_not_enabled', reason: 'Set IDENTITY_ENABLED=true to use this endpoint' },
        503,
      );
    }
    ...
    if (!identityService.supportsExport()) {
      return c.json(
        { error: 'not_supported', reason: 'Mock identity provider does not support registry export' },
        501,
      );
    }
  ...
  router.get('/settlements', (c) => {
    ...
    if (rawStatus && !validStatuses.includes(rawStatus as SettlementStatus)) {
      return c.json(
        { error: 'invalid_filter', reason: `status must be one of: ${validStatuses.join(', ')}` },
        400,
      );
    }

    const filter = rawStatus ? { status: rawStatus as SettlementStatus } : undefined;
  ...
  router.get('/settlements/:paymentId', (c) => {
    ...
    if (!record) {
      return c.json({ error: 'not_found', reason: `No settlement found for paymentId: ${paymentId}` }, 404);
    }
